...
*/
	...
	//withdraw ctoken to user's wallet when ctoken get stuck 
	function withdrawCTokens(
		...
		uint256 amount
 		...
 		address cEther = address(CEther);
   		...
   		if ( _ctoken == cEther ) {
    		IcEther(_ctoken).transfer(to, amount);	// if >0, send ctoken0 to user
		} else {
    		IcErc20(_ctoken).transfer(to, amount);	// if >0, send ctoken0 to user
		}
