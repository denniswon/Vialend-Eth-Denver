...
func redeemCToken() {
	...
	config.ChangeAccount(0)

	cTokenInstance := project.GetCTokenInstance(_ceth)

	camount, err := cTokenInstance.BalanceOf(&bind.CallOpts{}, config.FromAddress)

	fmt.Println("redeem for :", config.FromAddress)
	fmt.Println("redeem c amount :", camount)

	tx, err := cTokenInstance.Redeem(config.Auth, camount)
...
func redeemUnderlying(ethamount *big.Int) {
	...
	tx, err := cTokenInstance.RedeemUnderlying(config.Auth, ethamount)
	if err != nil {
