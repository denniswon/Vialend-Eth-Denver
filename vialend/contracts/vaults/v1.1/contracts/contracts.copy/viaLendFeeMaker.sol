...
//		Assetholder[to].deposittime.push(block.timestamp);
		...
		//do rebalance here for adding fees to existing users
		reCalcShares();
    ...
    function reCalcShares() internal {
		...
		prev0 = bal0.add(lastMint0).add(lastLending0);
		prev1 = bal1.add(lastMint1).add(lastLending1);
		...
		if (fees0 > 0 || fees1 > 0) {
			...
			if (_totalSupply > 0 ) {
				...
				for (uint i=0; i< cnt; i++) {
					...
					//mint the fee to the user which increase its share value
    ...
    function withdraw0() external  nonReentrant onlyTeam {
	...
	function _burnLendingShare (uint256 shares, uint256 totalShares) internal returns(uint256,uint256) {
		...
		removeLending(address(token0), address(token1),myamount0,myamount1);
...
*/    
	function removePositions() internal {
		...
		(lastMint0 ,lastMint1,lastLending0,lastLending1) = (0,0,0,0);
...
//		msg.sender.transfer(WETH.balanceOf(address(this)));
   ...
   //      uint256 amount0 = cAmount0.mul(10 ** (18 + 18 - 8)).div(CToken0.exchangeRateStored()) ;
   //      uint256 amount1 = cAmount1.mul(10 ** (18 + 18 - 8)).div(CToken1.exchangeRateStored()) ;
