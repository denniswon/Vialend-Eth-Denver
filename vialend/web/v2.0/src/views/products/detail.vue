    ...
    <div class="pair-intro">
      ...
      <div class="pair-title-right">
        <div class="token-icon">
          <cryptoicon v-if="pairInfo.token1.symbol !== ''" :symbol="pairInfo.token1.symbol" size="260" />
        </div>
      </div>
    </div>
    <hr />
...
const ViaVaultABI = require('../../abi/ViaVaultABI.json')
...
export default class extends Vue {
  ...
  async deposit() {
    ...
    console.log('deposit pair.vaultAddress:', pair.vaultAddress)

    console.log('deposit token0Amount:', token0Amount * Math.pow(10, pair.token0.decimals), ';token1Amount:', token1Amount * Math.pow(10, pair.token1.decimals))
  ...
  withdraw() {
    ...
    console.log('this.sharePercent=', this.sharePercent)
        ...
        .on('receipt', function(receipt:any) {
          if (_this.withdrawLoading === true) {
            ...
            if (receipt.status) {
              ...
              try {
                const pendingStatus = receipt.events.PendingWithdraw.returnValues.pending
                if (pendingStatus) {
                  _this.$message('Pending withdraw!')
                } else {
