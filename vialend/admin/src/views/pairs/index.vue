<template>
  <el-container>
    <el-tabs type="border-card">
      <el-tab-pane>
        <span slot="label"><i class="el-icon-date" /> Rebalance</span>
        <div v-loading="loadingTokensInfoStatus"
             style="width:80%">
        ...
        <el-button size="mini"
          ...
          <span :class="[dotStyle,rangeStatusStyle]" />
      ...
      <el-tab-pane>
        <span slot="label"><i class="el-icon-date" /> Security</span>
        <el-form :inline="true"
                 class="demo-form-inline">
      ...
      <el-tab-pane>
        <span slot="label"><i class="el-icon-date" /> Settings</span>
              ...
              <td>
                <el-form-item>
                  <el-input v-model="governanceAddress"
                            style="width:500px;" />&nbsp;&nbsp;<a :href="'https://goerli.etherscan.io/address/' + governanceAddress"
                     target="_blank"><img src="images/todetail.png"
                         width="20"></a>
                </el-form-item>
              </td>
              ...
              <td>
                <el-form-item>
                  <el-input v-model="teamAddress"
                            style="width:500px;" />&nbsp;&nbsp;<a :href="'https://goerli.etherscan.io/address/' + teamAddress"
                     target="_blank"><img src="images/todetail.png"
                         width="20"></a>
                </el-form-item>
              </td>
...
export default {
  ...
  data () {
    return {
      exchangeTimer: '',
      pairsList: store.state.pairsList,
      selectedPairIndex: 0,
  ...
  watch: {
    ...
    '$store.state.isConnected': function (newVal, oldVal) {
      ...
      this.isConnected = store.state.isConnected
      if (this.isConnected) {
  ...
  created: async function () {
    ...
    for (var i = 0; i < this.pairsList.size(); i++) {
      console.log('this.pairsList.get(i).token0.name=', this.pairsList.get(i).token0.name)
      console.log('this.pairsList.get(i).cLow=', this.pairsList.get(i).cLow)
      // console.log('pair feeTier=', this.pairsList.get(i).feeTier)
    }
    ...
    //   this.token0BalanceUSDInPool = Number(this.token0BalanceInPool) * Number(store.state.token0RateOfUSD)
    //   this.token1BalanceInPool = result.amount1 / Number(Math.pow(10, this.token1Decimal))
    //   this.token1BalanceUSDInPool = Number(this.token1BalanceInPool) * Number(store.state.token1RateOfUSD)
    // }
  },
  mounted () {
  ...
  methods: {
    ...
    getTokenRateOfUSD (symbol) {
      ...
      if (store.getters.getPriceUSDBySymbol(symbol) !== undefined) {
        // console.log('eth exchange table of ETH:', store.getters.getPriceUSDBySymbol(symbol).price_usd)
        return store.getters.getPriceUSDBySymbol(symbol).price_usd
      } else {
        return -1
      }
    },
    exchangeTokensIntoUSD () {
      if (store.state.tokenExchangeRateLoaded && this.pairsList.get(this.selectedPairIndex) !== undefined) {
      ...
      // console.log('tokenExchangeRateLoaded:', store.state.tokenExchangeRateLoaded)
    },
    async changeSelectedPair (index) {
      this.selectedPairIndex = index
      this.loadingTokensInfoStatus = true
      console.log('vault Address =====', this.pairsList.get(index).vaultAddress)
      this.vaultAddress = this.pairsList.get(index).vaultAddress
      var pair = await this.getPairBasicInfo(this.pairsList.get(index))
      this.poolAddress = pair.poolAddress
      this.token0Address = pair.token0.tokenAddress
      this.token1Address = pair.token1.tokenAddress
      this.token0Name = pair.token0.name
      this.token0Symbol = pair.token0.symbol
      this.token0Decimal = pair.token0.decimals
      this.token1Name = pair.token1.name
      this.token1Symbol = pair.token1.symbol
      this.token1Decimal = pair.token1.decimals
      this.tickLower = pair.cLow
      this.tickUpper = pair.cHigh
      this.token0LendingAddress = pair.token0.token0LendingAddress
      this.token1LendingAddress = pair.token1.token1LendingAddress
      this.initData()
    ...
    async loadPairsInfo () {
      ...
      this.token0LendingAddress = pair1.token0.token0LendingAddress
      this.token1LendingAddress = pair1.token1.token1LendingAddress
      ...
      // pair2.token1LendingAddress = '0x822397d9a55d0fefd20F5c4bCaB33C5F65bd28Eb'
    ...
    async loadTokensBalance () {
      ...
      console.log('this.token1Address=', this.token1Address)
      // ----------------------Tokens balance ------------------------------
      ...
      if (!isNaN(underlying0) && !isNaN(underlying1)) {
        ...
        this.token0BalanceUSDInLending = Number(this.token0BalanceInLending) * Number(store.state.token0RateOfUSD)
        ...
        this.token1BalanceUSDInLending = Number(this.token1BalanceInLending) * Number(store.state.token1RateOfUSD)
      }
    ...
    async getTokensBalanceInWallet () {
      ...
      if (this.isConnected) {
        ...
        this.token0BalanceUSDInWallet = Number(this.token0BalanceInWallet) * Number(store.state.token0RateOfUSD)
        ...
        this.token1BalanceUSDInWallet = Number(this.token1BalanceInWallet) * Number(store.state.token1RateOfUSD)
      }
      ...
      this.token0BalanceUSDInVault = Number(this.token0BalanceInVault) * Number(store.state.token0RateOfUSD)
      ...
      this.token1BalanceUSDInVault = Number(this.token1BalanceInVault) * Number(store.state.token1RateOfUSD)
    },
    async loadFees () {
        ...
        keeperUniswapV3Contract.methods
          ...
          .then(slot => {
            if (slot !== undefined && slot !== null && slot !== '') {
              ...
              _this.currentPrice = this.tickToPrice(_this.currentTick, _this.token0Decimal, _this.token1Decimal)
